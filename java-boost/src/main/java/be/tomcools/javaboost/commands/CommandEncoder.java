...
public class CommandEncoder {
    ...
    public String encodeMotorTime(Motor port, int seconds, int dutyCycle) {
        ...
        return String.format("0E0081%s110B%s%s647F03", port.getCode(), toLittleEndianHex(seconds * 1000, 4), convertDutyCycle(dutyCycle));
    }

    private String encodeMotorTimeMulti(Motor port, int seconds, int dutyCycle) {
        String dutyCicles = convertDutyCycle(dutyCycle);
        return String.format("0F0081%s110C%s%s%s647F03", port.getCode(), toLittleEndianHex(seconds * 1000, 4), dutyCicles, dutyCicles);
    }
    ...
    public String encodeMotorAngle(Motor port, int angle, int dutyCycle) {
        ...
        return String.format("0E0081%s110B%s%s647F03", port.getCode(), toLittleEndianHex(angle, 4), convertDutyCycle(dutyCycle));
    }
